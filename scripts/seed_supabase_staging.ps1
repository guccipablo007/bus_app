...
Push-Location $root
try {
    Write-Host 'Applying ordered seed files to Supabase staging.'
    & node '.\scripts\run_sql_files_with_pg.mjs' 'database/seeds'
    if ($LASTEXITCODE -ne 0) {
        throw 'Supabase seed runner failed.'
    }
    Write-Host 'Supabase staging seeds completed.'
} finally {
    Pop-Location
}
